...
export default function PortalDashboard({ publisher = false }: { publisher?: boolean }) {
  ...
  if (!publisher && user.role === 'INTERNAL_USER') {
    const internalBase = `/admin/internal_user/${user.id}`;
    const internalDashboard = `${internalBase}/dashboard`;
    if (location.pathname.startsWith('/admin/') && !location.pathname.startsWith('/admin/internal_user/'))
      return <Navigate to={`${internalBase}${location.pathname.slice('/admin'.length)}${location.search}`} replace/>;
    if (userId !== String(user.id) || !location.pathname.startsWith(`${internalBase}/`)) return <Navigate to={internalDashboard} replace/>;
    return <AdminWorkspace user={user} summary={summary} logout={logout}/>;
  }
  if (!publisher && user.role === 'SUPER_ADMIN') {
    if (location.pathname.startsWith('/admin/internal_user/')) return <Navigate to="/admin/dashboard" replace/>;
    return <AdminWorkspace user={user} summary={summary} logout={logout}/>;
  }
